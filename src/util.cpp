...
int nAnonymizeStyleCoinAmount = 1000;
...
#else
    const char* pszModule = "StyleCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\StyleCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\StyleCoin
    // Mac: ~/Library/Application Support/StyleCoin
    // Unix: ~/.StyleCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "StyleCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "StyleCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "StyleCoind.pid"));
