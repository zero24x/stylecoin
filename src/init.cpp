...
void Shutdown()
{
    ...
    RenameThread("StyleCoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: StyleCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: StyleCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeStyleCoinamount=<n> " + _("Keep N StyleCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. StyleCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. StyleCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("StyleCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "StyleCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of StyleCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart StyleCoin to complete") << "\n";
    ...
    nAnonymizeStyleCoinAmount = GetArg("-anonymizeStyleCoinamount", 0);
    if(nAnonymizeStyleCoinAmount > 999999) nAnonymizeStyleCoinAmount = 999999;
    if(nAnonymizeStyleCoinAmount < 2) nAnonymizeStyleCoinAmount = 2;
    ...
    LogPrintf("Anonymize StyleCoin Amount %d\n", nAnonymizeStyleCoinAmount);
