...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeStyleCoinAmount"))
        settings.setValue("nAnonymizeStyleCoinAmount", 1000);
    nAnonymizeStyleCoinAmount = settings.value("nAnonymizeStyleCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeStyleCoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeStyleCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeStyleCoinAmount:
            return QVariant(nAnonymizeStyleCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeStyleCoinAmount:
            nAnonymizeStyleCoinAmount = value.toInt();
            settings.setValue("nAnonymizeStyleCoinAmount", nAnonymizeStyleCoinAmount);
            emit AnonymizeStyleCoinAmountChanged(nAnonymizeStyleCoinAmount);
            break;
        default:
            break;
        }
