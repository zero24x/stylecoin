...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeStyleCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeStyleCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeStyleCoinAmount = strAnonymizeStyleCoinAmount.remove(strAnonymizeStyleCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStyleCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeStyleCoinAmount*COIN) nMaxToAnonymize = nAnonymizeStyleCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeStyleCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeStyleCoinAmount));
        strAnonymizeStyleCoinAmount = strAnonymizeStyleCoinAmount.remove(strAnonymizeStyleCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStyleCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeStyleCoinAmount == 0){
